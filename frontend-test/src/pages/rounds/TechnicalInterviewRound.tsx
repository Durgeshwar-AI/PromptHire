...
export function TechnicalInterviewRound() {
  ...
  const handleSubmit = () => { if (!submitted) setSubmitted(true); };

  const score = submitted
    ? QUESTIONS.reduce((acc, q, i) => (answers[i] === q.ans ? acc + 1 : acc), 0)
    : 0;
    ...
    <div className="h-screen flex flex-col bg-tertiary overflow-hidden">
      ...
      <nav className="flex items-center justify-between px-6 h-[56px] bg-secondary text-white shrink-0 z-20">
        ...
        <div className="flex items-center gap-5">
          ...
          {!submitted ? (
            <Btn size="sm" onClick={handleSubmit}>Submit Test</Btn>
          ) : (
      ...
      {submitted ? (
        <div className="flex-1 flex items-center justify-center p-8">
          <div className="text-center max-w-[480px]">
            <div className="text-6xl mb-4">{score >= 18 ? "🎉" : "📊"}</div>
            <div className="font-display font-black text-[clamp(2rem,4vw,3rem)] text-secondary uppercase mb-2">
              {score >= 18 ? "PASSED!" : "COMPLETED"}
            </div>
            <div className="font-body text-lg text-ink-light mb-6">
              You scored <strong className="text-secondary">{score}/{QUESTIONS.length}</strong>
              {" "}({Math.round((score / QUESTIONS.length) * 100)}%)
            </div>
            <div className={`border-2 px-6 py-4 mb-6 ${score >= 18 ? "border-[#1A8917] bg-[#f0fdf0]" : "border-[#c00] bg-[#fff5f5]"}`}>
              <div className="font-display font-extrabold text-sm uppercase">
                {score >= 18 ? "✅  Congratulations! You've completed all rounds." : "❌  Not selected"}
              </div>
            </div>
