...
function useDragSort(setItems: any) {
  ...
  const onDragStart  = useCallback((e: any, i: number) => { dragging.current = i; setDragIdx(i); e.dataTransfer.effectAllowed = "move"; }, []);
  const onDragEnter  = useCallback((e: any, i: number) => { e.preventDefault(); setOverIdx(i); }, []);
  ...
  const onDrop       = useCallback((e: any, i: number) => {
    ...
    if (from !== null && from !== i) setItems((p: any[]) => { const n=[...p]; const [m]=n.splice(from,1); n.splice(i,0,m); return n; });
...
export function PipelineBuilder({ onNavigate }: any) {
  ...
  const addRound    = (r: any) => { if (!selectedIds.includes(r.id)) { setPipeline(p => [...p, r]); setTimeout(() => flowRef.current?.scrollTo({ top: 9999, behavior: "smooth" }), 80); }};
  const removeRound = (id: string) => setPipeline(p => p.filter((r: any) => r.id !== id));
  ...
  return (
    <AppShell currentPage="pipeline" onNavigate={onNavigate}>
      ...
      <div className="fade-up mb-7">
        <p className="font-body text-xs tracking-[0.15em] uppercase text-primary mb-1">Configure Hiring Flow</p>
      ...
      <div className="fade-up flex items-center gap-3 pb-5 border-b border-border-clr flex-wrap mb-6">
        ...
        />
        {[{ k: "builder", l: "BUILDER" }, { k: "json", l: "{ } JSON" }].map((t) => (
          <button key={t.k} onClick={() => setTab(t.k)}
            className={[
              ...
              tab === t.k ? "bg-secondary text-white" : "bg-transparent text-secondary",
            ].join(" ")}>{t.l}</button>
        ))}
      </div>
      ...
      <div className="flex items-center gap-2.5 mb-5 bg-[#FEF0E8] border border-border-clr px-4 py-2 text-xs text-ink-light font-body">
        ...
        Hold the <strong className="text-primary">orange grip dots</strong> to drag &amp; reorder steps
      </div>
                ...
                return (
                  <button key={r.id} onClick={() => addRound(r)} disabled={sel}
                    ...
                    <div className="flex-1">
                      ...
                      {sel && <span className="text-[10px] text-primary ml-2 font-body">✓ Added</span>}
                      <div className="text-[10px] text-ink-faint font-body mt-px">{r.duration}</div>
                    </div>
                    {!sel && (
                      <div className="w-[22px] h-[22px] bg-secondary text-white flex items-center justify-center text-base font-bold">+</div>
                    )}
                  </button>
                );
              })}
          ...
          <div>
            ...
            {pipeline.length === 0 && (
              <div className="border-2 border-dashed border-border-clr py-14 px-5 text-center">
                <div className="font-display font-black text-2xl text-border-clr uppercase mb-2">NO ROUNDS YET</div>
                <p className="text-ink-faint text-xs font-body">Select rounds from the left</p>
              </div>
            )}
            ...
            <div ref={flowRef} className="flex flex-col items-center max-h-[520px] overflow-y-auto pb-1">
              {pipeline.map((round: any, i: number) => {
                const isDragging = drag.dragIdx === i;
                const isOver     = drag.overIdx === i && drag.dragIdx !== i;
                return (
                  <div key={round.id} className="w-full flex flex-col items-center">
                    <Connector />
                    <div draggable
                      ...
                      className={[
                        ...
                        isDragging ? "bg-[#FDF4EE] border-primary opacity-40" : "",
                        isOver ? "bg-[#FEF3EE] border-primary shadow-brutal-orange -translate-x-0.5 -translate-y-0.5" : "",
                        !isDragging && !isOver ? "bg-surface border-secondary" : "",
                      ].join(" ")}>
                      {/* grip */}
                      <div className="flex flex-col gap-[3px] px-1 py-0.5 opacity-50">
                        {[0,1,2].map(r => <div key={r} className="flex gap-[3px]">
                          {[0,1].map(c => <div key={c} className="w-[3.5px] h-[3.5px] rounded-full bg-primary" />)}
                        </div>)}
                      </div>
                      ...
                      <div className="flex-1">
                        <span className="font-display font-extrabold text-sm uppercase text-secondary">{round.label}</span>
                        <div className="text-[10px] text-ink-faint font-body mt-px">{round.duration}</div>
                      </div>
        ...
        <Card>
          ...
          <pre className="p-5 font-mono text-[11px] leading-[1.75] overflow-auto max-h-[400px] text-ink-light bg-surface-alt">
            {JSON.stringify({
              ...
              rounds: pipeline.map((r: any, i: number) => ({ round_number: i+1, round_id: r.id, label: r.label, agents: r.agents }))
            }, null, 2)}
          </pre>
        </Card>
      )}
